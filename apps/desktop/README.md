...
rin 桌面 GUI 壳 —— 一个 **Tauri 2 薄壳**，内嵌 rin 自己的 Web UI（`@rin/web`，8320），
...
选型定稿见根 `docs/archive/migration/MIGRATION.md` §8 决策 9/10。本包只做「壳」，业务能力全部在 host 侧
（`@rin/host` 装配 dsh-base + @rin 全家 + `@rin/host/web-server`），GUI 侧零业务逻辑。

## 架构图

```
┌────────────────────────────────────────────────────────────────────┐
│                        @rin/desktop  Tauri 2 薄壳                          │
│                                                                    │
│   ┌──────────────────────────────────────────────────────────────┐ │
...
│   │                                                              │ │
│   │    ┌────────────────────────────────────────────────────┐    │ │
│   │    │  rin web-ui（@rin/web，11 页 React SPA）        │    │ │
...
│   │  · 开发态：spawn `rin`（@rin/cli → @rin/host → host） │ │
│   │  · 发布态：spawn `rin-sidecar`（打包 runtime）   │ │
│   │  · GUI 退出事件回收 host；GUI 崩溃后的 sidecar 父进程看门狗仍待发布态实现与验证         │ │
│   └────────────────────────────────┬──────────────────────────────┘ │
│                                    │ node 子进程                     │
│   ┌────────────────────────────────▼──────────────────────────────┐ │
│   │  host = @rin/host 装配层                                      │ │
│   │  dsh-base + @rin 全家（repository/environment/...）+            │ │
│   │  @rin/host/web-server（默认 127.0.0.1:8320，enabled 开关）           │ │
│   └─────────────────────────────────────────────────────────────────┘ │
│                                                                    │
...
- **WebView 内嵌 8320 web-ui**：GUI 不打包、不 fork web-ui 的业务逻辑，只复用它的 11 页。
  开发态（`devUrl`）WebView 直连 `http://127.0.0.1:8320`，页面由 `@rin/host/web-server` 的
...
- **host 由 Rust 起收**：host 是独立 node 子进程。GUI 负责起它、接收 shell plugin 的输出/终止事件，并在退出时回收它；当前不在 Rust 侧等待 `/api/health`、自动重启或实现 sidecar 看门狗。前端 bootstrap 会单独探测健康状态并 fail-loud。
- **单 host 单端口**：默认只起一个 host、只占 8320，无双端口冲突、无性能浪费
  （docs/archive/migration/MIGRATION.md §8 决策 10）。

## 与双 webui 的关系

rin-harness 有三个 Web 面（docs/archive/migration/MIGRATION.md §4.1）。本包只属于其中之一：

| UI 面 | 端口 | 与 @rin/desktop 的关系 |
|---|---|---|
| **dsh 原生 Web UI** | 3080 | **独立、永不改动、GUI 不内嵌**。由 dsh 上游所有，用 `dsh --profile web` 原样启动；GUI 不 fork、不嵌、不关它的 bundle，也不读它 |
| **rin Web UI** | 8320 | rin 的主 Web 面，**GUI 内嵌它**（WebView 直连 8320）。可配置关闭（`@rin/host/web-server` 的 `enabled: false`），关闭只影响 8320，不影响任何 dsh 面 |
...
│   ├── tauri.conf.json           #   窗口/安全策略/host.externalBin/installerHooks
│   ├── capabilities/             #   Tauri capability 声明（当前仅 core/window；shell 由 Rust plugin 调用）
...
`identifier: "com.rin.harness"`、`devUrl: http://127.0.0.1:8320`、`frontendDist: ../../web/dist`、
`resources` 打进 builtin 仓库。sidecar：main.rs 以 `rin-sidecar[.exe]` 名 spawn（argv `web ...`）；
...
- **开发态（`cargo tauri dev`）**：Rust 侧 spawn `rin`（`@rin/cli` 的 bin），
  `@rin/cli` 装配 `@rin/host` 在 8320 起 host。需要本机已 `pnpm install` +
...
- **发布态（`cargo tauri build`）**：Rust 侧 spawn 随安装包分发的 sidecar
  ...
  **无需安装 Node/pnpm**；argv = `web --port 8320 --host 127.0.0.1`）。打包方式见 `scripts/build-sidecar.md`。

## 实现现状（子任务 1：Tauri 配置 + Rust 壳 + 前端 bootstrap）

已落地的骨架（子任务 1 产物，真机构建前为静态自查级）：

- Rust 侧使用 Tauri 官方 `tauri-plugin-shell`：统一 `start_host()`，开发态调用 `app.shell().command(...)`，发布态调用 `app.shell().sidecar("rin-sidecar")`。
  - 开发态 argv：`rin web --port 8320 --host 127.0.0.1`（命令名可用 `RIN_GUI_HOST_CMD` 覆盖）。
  - 发布态 argv：`rin-sidecar web --port 8320 --host 127.0.0.1`（sidecar 名对齐 `scripts/build-sidecar.md`）。
  - 两种入口统一带 `RIN_PARENT_PID` 环境变量；sidecar 看门狗仍需随发布态构建落地并在真机验证。
  - stdout/stderr/错误/终止事件由 shell plugin 异步转发到日志；退出时通过 `CommandChild.kill()` 回收 host。
- 托盘：左键单击显示窗口，菜单「Show rin / Quit rin」；关闭窗口隐藏到托盘而非退出；托盘退出置 quitting 标志后 app.exit(0)。
- `build.frontendDist` = "../../web/dist"：发布态直接打包 `@rin/web` 的产物，与浏览器 8320 共用同一套 UI。
...
```sh
# rin web-ui 静态产物（供 @rin/host/web-server staticRoot 伺服，也供 GUI 发布态打包）
pnpm install
pnpm run build:web

# 开发态 host：pnpm run start（等价于 source-launch @rin/cli -> @rin/host -> 8320）
pnpm run start
```
...
- **LLM 依赖面**：`@rin/workspace/agents` 的 AI 提案、`@rin/evolution` 的 reviewModel 走 dsh
  llm seam，需真机 `DEEPSEEK_API_KEY` 才能端到端验证（对齐 @rin/web 已知限制）。

## 关联文档

- 根 `docs/archive/migration/MIGRATION.md` §4.1（UI 三面共存）、§8 决策 9/10（GUI 选型 / 装配零侵入）。
...
- `@rin/host/web-server` README —— 8320 JSON API 契约（GUI 内嵌页面的数据源）。
- `@rin/web` README —— 11 页清单。
- `@rin/cli` / `@rin/host` README —— 开发态 host 的装配与启动。
