...
import { selectClass as sharedSelectClass } from '../../../components/shared/styles'
import { PromptEditor } from './PromptEditor'
import { DayOfWeekPicker } from './DayOfWeekPicker'
import { useTranslation } from '../../../i18n'
import { describeCron, isValidCron, parseCron, type FrequencyKey } from '../cronDescribe'
...
export function NewTaskModal({ open, onClose, editTask }: Props) {
  ...
  const timeValid = !showTime || /^\d{1,2}:\d{2}$/.test(time)
  ...
  const selectClass = sharedSelectClass
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[13px] font-semibold text-[var(--color-text-primary)]">{t('newTask.frequency')}</label>
        ...
        {frequency === 'customCron' && (
          <div className="flex flex-col gap-1">
            <input
              ...
              className="h-10 w-full rounded-[10px] border border-[var(--color-border)] bg-[var(--color-surface-container-low)] px-3 font-mono text-[13px] text-[var(--color-text-primary)] outline-none focus:border-[var(--color-border-focus)] focus:shadow-[var(--shadow-focus-ring)]"
            />
        ...
        {showTime && (
          <div className="flex flex-col gap-1">
            <input
              ...
              className="h-10 w-auto rounded-[10px] border border-[var(--color-border)] bg-[var(--color-surface-container-low)] px-3 text-[13px] font-medium text-[var(--color-text-primary)] outline-none focus:border-[var(--color-border-focus)] focus:shadow-[var(--shadow-focus-ring)]"
              style={{ maxWidth: 120 }}
            />
          </div>
        )}
        ...
        <div className="flex flex-col gap-3 rounded-[12px] border border-[var(--color-border)] p-3">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <div>
              <span className="text-[13px] font-semibold text-[var(--color-text-primary)]">{t('newTask.notifyOnComplete')}</span>
            ...
            <div className="flex flex-col gap-2 pl-7">
              <div className="flex items-center gap-4">
                <label className={`flex items-center gap-2 ${isFeishuConfigured ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}>
                  ...
                  />
                  <span className="text-[13px] text-[var(--color-text-primary)]">{t('settings.adapters.feishu')}</span>
                  {!isFeishuConfigured && (
                    <span className="text-[11px] text-[var(--color-warning)]">{t('newTask.notConfigured')}</span>
                  )}
                </label>
                <label className={`flex items-center gap-2 ${isTelegramConfigured ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}>
                  <input
                    ...
                    className="w-3.5 h-3.5 rounded border-[var(--color-border)] accent-[var(--color-brand)]"
                  />
                  <span className="text-[13px] text-[var(--color-text-primary)]">{t('settings.adapters.telegram')}</span>
