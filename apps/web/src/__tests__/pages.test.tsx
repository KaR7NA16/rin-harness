...
describe('Content-only pages render without errors', () => {
  it('EmptySession renders the shared new-session chooser', async () => {
    ...
    expect(screen.getByRole('group', { name: 'New session' })).toBeInTheDocument()
    expect(await screen.findByRole('button', { name: 'Choose folder...' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Temporary session' })).toBeInTheDocument()
  ...
  it('EmptySession opens a temporary session from the chooser', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Temporary session' }))
  ...
  it('ScheduledTasks renders (store-connected)', async () => {
    ...
    await screen.findByText(/Run tasks on a schedule/)
    expect(container.innerHTML).toContain('/schedule')
  })
})
