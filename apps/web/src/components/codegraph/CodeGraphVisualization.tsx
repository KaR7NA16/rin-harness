...
const EDGE_STYLES_DARK: Record<string, { color: string; dash: number[] }> = {
  calls: { color: '#45e1bc', dash: [] },
  references: { color: '#ffba63', dash: [4, 5] },
  contains: { color: '#6f8794', dash: [] },
}

const EDGE_STYLES_LIGHT: Record<string, { color: string; dash: number[] }> = {
  calls: { color: '#0d9e8a', dash: [] },
  references: { color: '#e08a1e', dash: [4, 5] },
  contains: { color: '#93a5ae', dash: [] },
}

type GraphPalette = {
  kindColors: Record<string, string>
  edgeStyles: Record<string, { color: string; dash: number[] }>
...
function useGraphPalette(): GraphPalette {
  ...
  return useMemo(
    () => isDark
      ? { kindColors: KIND_COLORS_DARK, edgeStyles: EDGE_STYLES_DARK, fallbackColor: '#78909a' }
      : { kindColors: KIND_COLORS_LIGHT, edgeStyles: EDGE_STYLES_LIGHT, fallbackColor: '#607d8b' },
    [isDark],
  )
}
...
export function CodeGraphVisualization({ data }: { data: CodeGraphData }) {
  ...
  }, [canvasSize, data, hoveredId, layout, nodeById, normalizedQuery, palette, selectedId, transform])
  ...
  const clampPan = useCallback((next: ViewTransform): ViewTransform => {
    const scale = getFitScale(canvasSize, next.scale)
    const margin = 60
    const minX = margin - canvasSize.width / 2 - (WORLD_WIDTH / 2) * scale
    const maxX = canvasSize.width / 2 - margin + (WORLD_WIDTH / 2) * scale
    const minY = margin - canvasSize.height / 2 - (WORLD_HEIGHT / 2) * scale
    const maxY = canvasSize.height / 2 - margin + (WORLD_HEIGHT / 2) * scale
    return { scale: next.scale, x: clamp(next.x, minX, maxX), y: clamp(next.y, minY, maxY) }
  }, [canvasSize])
  ...
  }, [canvasSize, transform.scale, clampPan])
  ...
  const handleCanvasKeyDown = (event: React.KeyboardEvent<HTMLCanvasElement>) => {
    ...
    if (event.key === 'ArrowLeft' || event.key === 'ArrowRight' || event.key === 'ArrowUp' || event.key === 'ArrowDown') {
      ...
      setTransform((current) => clampPan({ ...current, x: current.x + dx, y: current.y + dy }))
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-[12px]">
      ...
      <div
        ref={containerRef}
        className="relative min-h-[440px] w-full flex-1 overflow-hidden rounded-[10px] border border-[var(--color-graph-border)] bg-[var(--color-graph-bg)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),inset_0_0_0_1px_var(--color-graph-inset)]"
      >
        <canvas
          ref={canvasRef}
          tabIndex={0}
          ...
          className="block cursor-grab touch-none outline-none focus-visible:shadow-[inset_0_0_0_2px_var(--color-border-focus)] active:cursor-grabbing"
        ...
        <div className="pointer-events-none absolute left-[12px] top-[11px] flex items-center gap-[8px] font-mono text-[9px] uppercase text-[var(--color-graph-text-muted)]">
        ...
        {layout.nodes.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center text-[13px] text-[var(--color-graph-text-muted)]">
...
function GraphLegend({ t }: { t: ReturnType<typeof useTranslation> }) {
  return (
    <div className="pointer-events-none absolute bottom-[11px] left-[12px] flex flex-wrap items-center gap-x-[13px] gap-y-[5px] bg-[var(--color-graph-panel-bg)] px-[7px] py-[5px] font-mono text-[9px] text-[var(--color-graph-text-muted)] backdrop-blur-[4px]">
...
function NodeTooltip({ node, x, y, size, t }: {
  ...
  return (
    <div
      className="pointer-events-none absolute z-10 max-w-[280px] border border-[var(--color-graph-tooltip-border)] bg-[var(--color-graph-tooltip-bg)] px-[10px] py-[8px] font-mono text-[10px] text-[var(--color-graph-text)] shadow-[0_12px_30px_rgba(0,0,0,0.4)] backdrop-blur-[6px]"
      style={{
        left: Math.max(8, Math.min(x + 14, size.width - 294)),
        top: Math.max(8, Math.min(y - 12, size.height - 62)),
      }}
    >
      <div className="truncate font-semibold text-[var(--color-graph-text)]">{node.qualifiedName || node.name}</div>
      <div className="mt-[3px] truncate text-[var(--color-graph-text-muted)]">{node.filePath}:{node.startLine}</div>
      <div className="mt-[3px] truncate text-[var(--color-graph-text-secondary)]">
...
function NodeInspector({ node, connections, onClose, onSelect, t, palette }: {
  node: PositionedNode
  connections: Array<{
    node: PositionedNode
    kind: string
  ...
  return (
    <aside className="absolute bottom-0 right-0 top-0 z-20 flex w-[min(292px,78%)] flex-col border-l border-[var(--color-graph-inspector-border)] bg-[var(--color-graph-inspector-bg)] text-[var(--color-graph-text)] shadow-[-18px_0_44px_rgba(0,0,0,0.28)] backdrop-blur-[10px]">
      <header className="flex items-start justify-between gap-[12px] border-b border-[var(--color-graph-border)] px-[16px] py-[15px]">
        <div className="min-w-0">
          <div className="flex items-center gap-[7px] font-mono text-[9px] uppercase text-[var(--color-graph-text-muted)]">
            <span className="h-[6px] w-[6px] rounded-full" style={{ background: palette.kindColors[node.kind] || palette.fallbackColor }} />
            {node.kind} · {getRoleLabel(node.role, t)}
          </div>
          <h2 className="mt-[7px] truncate text-[14px] font-semibold text-[var(--color-graph-text)]">
        ...
        <button
          ...
          onClick={onClose}
          className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[6px] text-[var(--color-graph-text-muted)] hover:bg-[var(--color-graph-hover-bg)] hover:text-[var(--color-graph-hover-text)]"
        >
          <X size={15} />
        </button>
      </header>

      <div className="border-b border-[var(--color-graph-border)] px-[16px] py-[13px] font-mono text-[10px] leading-[1.65]">
        <div className="break-all text-[var(--color-graph-text-secondary)]">{node.filePath}</div>
        <div className="mt-[5px] flex items-center justify-between text-[var(--color-graph-text-muted)]">
          <span>{t('tokenOptimization.graph.inspector.lines')}</span>
          <span className="text-[var(--color-graph-text)]">{node.startLine}-{node.endLine}</span>
        </div>
        <div className="flex items-center justify-between gap-[12px] text-[var(--color-graph-text-muted)]">
          <span>{t('tokenOptimization.graph.inspector.community')}</span>
          <span className="truncate text-right text-[var(--color-graph-text)]">{node.communityLabel}</span>
        </div>
        <div className="flex items-center justify-between text-[var(--color-graph-text-muted)]">
          <span>{t('tokenOptimization.graph.inspector.degree')}</span>
          <span className="text-[var(--color-graph-text)]">{node.degree}</span>
        </div>
      </div>

      {grouped.length > 0 && (
        <div className="flex flex-wrap gap-x-[14px] gap-y-[5px] border-b border-[var(--color-graph-border)] px-[16px] py-[11px] font-mono text-[9px] uppercase text-[var(--color-graph-text-muted)]">
          {grouped.map(([kind, count]) => (
            <span key={kind}><strong className="mr-[4px] text-[var(--color-graph-text)]">{count}</strong>{kind}</span>
          ))}
        </div>
      )}

      <div className="min-h-0 flex-1 overflow-y-auto px-[8px] py-[10px]">
        <div className="px-[8px] pb-[7px] font-mono text-[9px] uppercase text-[var(--color-graph-text-muted)]">
          {t('tokenOptimization.graph.inspector.related')}
        </div>
        {connections.length === 0 ? (
          <div className="px-[8px] py-[12px] text-[11px] text-[var(--color-graph-text-faint)]">
          ...
          <button
            ...
            className="group flex w-full items-center gap-[9px] border-b border-[var(--color-graph-border)] px-[8px] py-[9px] text-left hover:bg-[var(--color-graph-hover-bg)]"
          >
            <span
              className="h-[6px] w-[6px] shrink-0 rounded-full"
              style={{ background: palette.kindColors[connection.node.kind] || palette.fallbackColor }}
            />
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[11px] text-[var(--color-graph-text)] group-hover:text-[var(--color-graph-hover-text)]">
                {connection.node.name}
              </span>
              <span className="mt-[2px] block truncate font-mono text-[8px] uppercase text-[var(--color-graph-text-faint)]">
                {connection.direction === 'out' ? '→' : '←'} {connection.kind} · {getConfidenceLabel(connection.confidence, t)}
                {connection.line ? ` · L${connection.line}` : ''}
              </span>
            </span>
            <Focus size={12} className="shrink-0 text-[var(--color-graph-icon)] opacity-0 group-hover:opacity-100" />
          </button>
        ))}
      </div>
    </aside>
...
function drawGraph({ context, size, transform, data, layout, nodeById, hoveredId, selectedId, query, palette, time }: {
  ...
  const { kindColors, edgeStyles, fallbackColor } = palette
  ...
  for (const edge of data.edges) {
    ...
    const style = edgeStyles[edge.kind] || edgeStyles.contains!
  ...
  for (const node of layout.nodes) {
    ...
    const color = kindColors[node.kind] || fallbackColor
    context.save()
