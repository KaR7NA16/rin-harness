...
import { NotePickerModal } from '../notes/NotePickerModal'
...
export function ChatInput({ variant = 'default', sessionId: sessionIdProp, projectPath, onSubmit: onSubmitProp, workDir: workDirProp, onWorkDirChange, runtimeKey, isPanelActive = true }: ChatInputProps) {
  ...
  const [input, setInput] = useState(() => {
    const sid = sessionIdProp ?? useTabStore.getState().activeTabId
    if (!sid || useTeamStore.getState().getMemberBySessionId(sid)) return ''
    return readComposerDraft(sid)
  })
  ...
  const draftSessionId = activeTabId && !isMemberSession ? activeTabId : null
  const restoredDraftRef = useRef<string | null>(draftSessionId)
  const latestDraftRef = useRef<{ sessionId: string | null; value: string }>({ sessionId: draftSessionId, value: input })
  latestDraftRef.current = { sessionId: draftSessionId, value: input }

  useEffect(() => {
    if (!draftSessionId || restoredDraftRef.current === draftSessionId) return
    restoredDraftRef.current = draftSessionId
    const draft = readComposerDraft(draftSessionId)
    setInput(draft || '')
  }, [draftSessionId])
  ...
  useEffect(() => {
    ...
    if (!input) {
      writeComposerDraft(draftSessionId, '')
      return
    }
    const timer = window.setTimeout(() => writeComposerDraft(draftSessionId, input), COMPOSER_DRAFT_SAVE_DELAY_MS)
    return () => window.clearTimeout(timer)
  }, [draftSessionId, input])

  useEffect(() => () => {
    const { sessionId, value } = latestDraftRef.current
    if (sessionId && value) writeComposerDraft(sessionId, value)
  }, [])
  ...
  useEffect(() => {
    ...
    let cancelled = false
    sessionsApi
      .getGitInfo(activeTabId, { projectPath })
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    // Escape closes the plus menu before it can fall through to stopping generation
    ...
    // Escape stops generation only when no floating panel consumed it
  ...
  const handleCompactContext = () => {
    if (isMemberSession || !activeTabId) return
    setPlusMenuOpen(false)
    setSlashMenuOpen(false)
    setFileSearchOpen(false)
  ...
  const handleProjectFolderSelect = useCallback(async () => {
    ...
    try {
      ...
      if (isTauriRuntime()) {
        ...
        useUIStore.getState().addToast({
          type: 'info',
          message: t('dirPicker.desktopOnly'),
        })
        return
      }
  ...
  const handleVoiceInput = () => {
    useUIStore.getState().addToast({
      message: t('chat.voiceInputUnavailable'),
      type: 'info',
      duration: 3000,
    })
  }
  ...
  const insertNoteReference = (title: string, path: string, noteContent: string) => {
    const el = textareaRef.current
    const cursorPos = el?.selectionStart ?? input.length
    const block = `<note title="${title}" path="${path}">\n${noteContent}\n</note>\n`
    const newValue = `${input.slice(0, cursorPos)}${block}${input.slice(cursorPos)}`
    setInput(newValue)
  ...
  const compactContextLabel = t('chat.compactContext')
  const reviewContextLabel = t('chat.reviewChanges')
  ...
  return (
    <div className={isHeroComposer ? '' : 'pointer-events-none flex justify-center p-[24px]'}>
      ...
      >
        {notePickerOpen && (
          <NotePickerModal
            onSelect={doc => insertNoteReference(doc.title, doc.path, doc.content)}
        ...
        {!isMemberSession && slashMenuOpen && filteredCommands.length > 0 && (
          ...
          >
            ...
            <div className="mt-[6px] flex items-center gap-1.5 rounded-[16px] bg-[var(--color-surface-container-low)] px-[10px] py-[8px] text-[10px] font-medium text-[var(--color-text-tertiary)]">
              <kbd className={`${kbdChipClass}`}>↑↓</kbd>
              <span>{t('chat.navigate')}</span>
              <kbd className={`ml-2 ${kbdChipClass}`}>↵</kbd>
              <span>{t('chat.select')}</span>
              <kbd className={`ml-2 ${kbdChipClass}`}>Esc</kbd>
                ...
                {!isMemberSession && (
                  <>
                    ...
                    >
                      ...
                      <Tooltip label={slashCommandsLabel} />
                    </button>
                    ...
                    {plusMenuOpen && (
                      <div className="absolute bottom-full left-0 z-50 mb-[10px] w-[260px] overflow-hidden rounded-[24px] border-2 border-[var(--color-border)] bg-[var(--color-surface-container-lowest)] p-[8px] shadow-[var(--shadow-dropdown)]">
                        ...
                        <button
                          onClick={() => { setPlusMenuOpen(false); setNotePickerOpen(true) }}
                          className="group flex min-h-[54px] w-full items-center gap-[10px] rounded-[16px] px-[10px] py-[8px] text-left transition-colors hover:bg-[var(--color-surface-hover)]"
                        >
                          <span className="flex h-[34px] w-[34px] shrink-0 items-center justify-center rounded-full border border-[var(--color-border-separator)] bg-[var(--color-surface-container)] font-mono text-[13px] font-semibold text-[var(--color-text-secondary)]">N</span>
                          <span className="min-w-0 flex-1">
                            <span className="block truncate text-[13px] font-semibold text-[var(--color-text-primary)]">{t('chat.addNoteRef')}</span>
                            <span className="mt-[2px] block truncate text-[11px] font-medium text-[var(--color-text-tertiary)]">{t('sidebar.notes')}</span>
                          </span>
                        </button>
                      </div>
                    )}
              ...
              <div className="relative flex items-center">
                {!isMemberSession && (
                  ...
                  >
                    ...
                    />
                    <Tooltip label={captureScreenshotLabel} />
                  </button>
                )}
              </div>
              ...
              <div className="relative flex items-center">
                {!isMemberSession && (
                  ...
                  >
                    ...
                    <Tooltip label={addProjectFolderLabel} />
                  </button>
                )}
              </div>
              ...
              {!isMemberSession && (
                <div className="relative flex items-center">
                  ...
                  >
                    ...
                    <Tooltip label={compactContextLabel} />
                  </button>
                </div>
              )}
              ...
              {!isMemberSession && (
                <div className="relative flex items-center">
                  <button
                    ...
                    aria-label={reviewContextLabel}
                    className={composerToolButtonClassName}
                  >
                    <FileDiff size={16} strokeWidth={2.25} />
                    <Tooltip label={reviewContextLabel} />
                  </button>
                </div>
              )}
            ...
            <div data-testid="composer-runtime-controls" className="ml-auto flex min-w-0 items-center justify-end gap-[8px] overflow-visible">
              {!isMemberSession && (
                <div className="relative flex items-center">
                  ...
                  >
                    ...
                    <Tooltip label={t('chat.voiceInput')} />
                  </button>
                </div>
              )}
              ...
              {runtimeKey && !isMemberSession && (
                <div
                  ...
                  title={isActive ? t('chat.modelSelectorBusy') : undefined}
                >
                  <ModelSelector
                ...
                <>
                  {isActive ? (
                    <button
                      ...
                      className="flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-full bg-[var(--color-surface-container-high)] text-[var(--color-text-secondary)] transition-colors duration-100 hover:bg-[var(--color-error)] hover:text-[var(--color-btn-danger-fg)]"
                    >
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          if (activeTabId) sendMessage(activeTabId, '/compact')
        }}
        title={t('chat.compactConfirmTitle')}
        body={t('chat.compactConfirmBody')}
        confirmLabel={t('chat.compactConfirmLabel')}
        cancelLabel={t('common.cancel')}
        confirmVariant="danger"
      />

      {reviewOpen && sessionState && (
        <SessionReviewView
