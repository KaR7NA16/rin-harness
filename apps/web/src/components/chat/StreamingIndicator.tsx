...
import { SERVER_VERB_KEYS } from '../../config/serverVerbs'
...
function fallbackVerbKey(chatState: ChatState): TranslationKey {
  if (chatState === 'permission_pending') return 'streaming.awaitingApproval'
...
export function resolveStreamingVerb(
  ...
  if (chatState === 'permission_pending') return translate('streaming.awaitingApproval')
...
export function StreamingIndicator({ sessionId }: StreamingIndicatorProps = {}) {
  ...
  const isPermissionPending = chatState === 'permission_pending'
  const dotColor = isPermissionPending ? 'bg-[var(--color-warning)]' : 'bg-[var(--color-brand)]'
    ...
    >
      <div data-chat-content-column className="flex h-[40px] w-full max-w-[878px] items-center gap-2 overflow-hidden px-[20px]">
        <span className="relative flex h-1.5 w-1.5 shrink-0" aria-hidden="true">
          <span className={`breathing-dot-ring absolute inline-flex h-full w-full rounded-full ${dotColor} opacity-30`} />
          <span className={`relative inline-flex h-1.5 w-1.5 rounded-full ${dotColor}`} />
        </span>

        <span data-testid="streaming-verb" className={`ai-shimmer-text inline-flex h-[18px] min-w-0 items-center truncate whitespace-nowrap text-[12px] font-medium leading-[18px] ${isPermissionPending ? 'text-[var(--color-warning)]' : ''}`}>
          {verb}
        </span>
