...
describe('chat blocks', () => {
  ...
  it('shows a stable color identifier on Agent subtask cards', () => {
    const makeAgent = (description: string) => ({
      id: description,
      type: 'tool_use' as const,
    ...
    const toolCalls = [makeAgent('探索整体架构'), makeAgent('解析布局')]
    ...
    const dots = first.container.querySelectorAll('[style*="background-color"]')

    expect(dots.length).toBeGreaterThanOrEqual(1)
    expect(dots[0]?.getAttribute('style')).toMatch(/background-color/)
    ...
    const firstDotColor = dots[0]!.getAttribute('style')
    const repeatedDot = second.container.querySelector('[style*="background-color"]')
    expect(repeatedDot?.getAttribute('style')).toBe(firstDotColor)
  })
})
