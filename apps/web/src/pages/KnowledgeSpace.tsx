...
export function KnowledgeSpace() {
  ...
  const hasIndexingSourcesRef = useRef(false)
  ...
  hasIndexingSourcesRef.current = sources.some(
    (source) => source.status === 'pending' || source.status === 'indexing',
  )

  useEffect(() => {
    const timer = window.setInterval(() => {
      if (hasIndexingSourcesRef.current) void loadSources(true)
    }, 1_000)
  ...
  }, [mode, notifyError, selectedSourceId, t])
  ...
  const chooseSources = useCallback(async (directory: boolean) => {
    try {
      ...
      useUIStore.getState().addToast({
        type: 'info',
        message: t('knowledgeSpace.sources.pickerUnavailable'),
      })
    } catch (error) {
      notifyError(error, t('knowledgeSpace.errors.add'))
    }
    ...
    >
      ...
      <main className="flex min-w-0 flex-1 flex-col bg-[var(--color-background)]">
        ...
        <div className={`min-h-0 flex-1 px-[16px] pb-[16px] pt-[14px] md:px-[20px] ${mode === 'graph' ? 'flex flex-col overflow-hidden' : 'overflow-auto'}`}>
          ...
          {mode === 'files' && (
            <FilesWorkspace documents={documents} loading={documentsLoading} />
          )}
          {mode === 'search' && (
            <SearchWorkspace
              query={searchQuery}
      ...
      <ConfirmDialog
        open={removeConfirmOpen && selectedSource !== null}
        onClose={() => setRemoveConfirmOpen(false)}
        onConfirm={() => {
          setRemoveConfirmOpen(false)
          void removeSelectedSource()
        }}
        title={t('knowledgeSpace.details.remove')}
        body={t('knowledgeSpace.details.removeConfirm', { name: selectedSource?.name ?? '' })}
        confirmLabel={t('knowledgeSpace.details.remove')}
