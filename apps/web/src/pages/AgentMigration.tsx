...
import { MigrationStepper } from '../components/settings/MigrationStepper'
...
export function AgentMigration() {
  ...
  const [overwriteConfirm, setOverwriteConfirm] = useState<{
    input: { itemIds?: string[]; projectIds?: string[]; allRecommended?: boolean }
    busyKey: string
  ...
  const migrationStep = migrating ? 4 : loading ? 1 : selectionCount > 0 ? 2 : 0
  ...
  const countOverwrites = (input: {
    itemIds?: string[]
    projectIds?: string[]
    allRecommended?: boolean
  }) => {
    if (!activeAgent) return 0
    const overwrites = (item: AgentMigrationItem) =>
      item.destinationState === 'exists' || item.destinationState === 'conflict'
    if (input.allRecommended) {
      return activeAgent.items.filter(item =>
        item.scope === 'global' && item.recommended && item.selectable && overwrites(item),
      ).length
    }
    const itemIds = new Set(input.itemIds ?? [])
    const projectItemIds = new Set(
      activeAgent.projects
        .filter(project => (input.projectIds ?? []).includes(project.id))
        .flatMap(project => project.itemIds),
    )
    return activeAgent.items.filter(item =>
      (itemIds.has(item.id) || projectItemIds.has(item.id)) && overwrites(item),
    ).length
  }

  const requestMigration = (input: {
    itemIds?: string[]
    projectIds?: string[]
    allRecommended?: boolean
  }, busyKey: string) => {
    const overwriteCount = countOverwrites(input)
    if (overwriteCount > 0) {
      setOverwriteConfirm({ input, busyKey, overwriteCount })
      return
    }
    void runMigration(input, busyKey)
  }
  ...
  return (
    ...
    >
      <MigrationStepper
        steps={['选择来源', '扫描数据', '选择内容', '预览冲突', '完成迁移']}
          ...
          <aside className="border-b border-[var(--color-border-separator)] bg-[var(--color-surface-container-low)] md:border-b-0 md:border-r">
            <div className="flex h-[50px] items-center justify-between border-b border-[var(--color-border-separator)] px-[14px]">
              ...
              <button
                ...
                className="flex h-[32px] w-[32px] items-center justify-center rounded-full text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)] disabled:opacity-40"
              >
            ...
            {loading ? (
              ...
              <>
                <AgentHeader
                  ...
                  onMigrateRecommended={() => requestMigration({ allRecommended: true }, 'recommended')}
                />

                <div className="flex flex-wrap items-center justify-between gap-[10px] border-b border-[var(--color-border-separator)] px-[20px] py-[10px]">
                ...
                <div className="max-h-[430px] min-h-[280px] overflow-y-auto">
                  {filter === 'project' ? (
                    ...
                    <MigrationItemList
                      ...
                      onMigrate={item => requestMigration({ itemIds: [item.id] }, `item:${item.id}`)}
                    />
                  )}
                </div>

                <footer className="flex min-h-[64px] flex-wrap items-center justify-between gap-[10px] border-t border-[var(--color-border-separator)] px-[20px] py-[10px]">
      ...
      {error && (
        <div role="alert" className="rounded-[12px] border border-[var(--color-error)]/25 bg-[var(--color-error)]/5 px-[14px] py-[11px] text-[12px] text-[var(--color-error)]">
          {error}
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          if (pending) void runMigration(pending.input, pending.busyKey)
        }}
        title={t('agentMigration.overwriteTitle')}
        body={t('agentMigration.overwriteBody', { count: overwriteConfirm?.overwriteCount ?? 0 })}
        confirmLabel={t('agentMigration.migrateOverwrite')}
...
function MigrationRoute({
  ...
  const t = useTranslation()
  return (
    <div className="grid min-h-[64px] grid-cols-[minmax(0,1fr)_40px_minmax(0,1fr)] items-end gap-[10px] border-b border-[var(--color-border-separator)] bg-[var(--color-surface-container-low)] px-[20px] py-[10px]">
      <div className="min-w-0">
        <span className="block text-[11px] font-semibold text-[var(--color-text-tertiary)]">
      ...
      <button
        ...
        className="flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-[8px] border border-[var(--color-border)] bg-[var(--color-background)] text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)] disabled:cursor-not-allowed disabled:opacity-35"
      >
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex min-w-0 items-center gap-[6px]">
                    <span className="truncate text-[13px] font-semibold text-[var(--color-text-primary)]">{agent.name}</span>
                    {isDefault && (
                      <span className="shrink-0 rounded-[4px] bg-[var(--color-surface-container-low)] px-[5px] py-[1px] text-[11px] font-semibold text-[var(--color-text-tertiary)]">
                        {t('agentMigration.defaultTarget')}
                      </span>
                    )}
                  </span>
                  <span className="mt-[1px] block truncate text-[11px] text-[var(--color-text-tertiary)]">
                    {isSource
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-[2px] block truncate text-[11px] text-[var(--color-text-tertiary)]">
          {isTarget
      ...
      <div className="flex min-w-0 items-center gap-[11px]">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-[8px]">
            <h2 className="truncate text-[15px] font-semibold text-[var(--color-text-primary)]">{agent.name}</h2>
            <span className={`rounded-full px-[7px] py-[2px] text-[11px] font-semibold ${
              agent.installed
          ...
          <p className="mt-[4px] truncate font-mono text-[11px] text-[var(--color-text-tertiary)]" title={agent.executablePath ?? agent.dataRoots[0]}>
        ...
        const destinationLabel = item.destinationState === 'merge'
          ...
          : item.destinationState === 'exists'
            ...
            : item.destinationState === 'conflict'
              ...
              : null
        return (
          <div key={item.id} className="flex min-w-0 items-center gap-[10px] px-[20px] py-[12px] min-h-[56px] hover:bg-[var(--color-surface-hover)]/55">
            <input
              type="checkbox"
            ...
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 items-center gap-[7px]">
                ...
                <span
                  ...
                  className={`shrink-0 rounded-[4px] px-[5px] py-[1px] text-[11px] font-semibold ${
                ...
                {item.selectionIssue === 'size-limit' && (
                  <span className="shrink-0 text-[11px] font-semibold text-[var(--color-warning)]">
                    {t('agentMigration.sizeLimit')}
                  </span>
                )}
              </div>
              <div className="mt-[3px] flex min-w-0 items-center gap-[8px] text-[11px] text-[var(--color-text-tertiary)]">
  ...
  return (
    <div className="divide-y divide-[var(--color-border-separator)]">
      {projects.map(project => {
        ...
        return (
        <div key={project.id} className="flex min-w-0 items-center gap-[10px] px-[20px] py-[12px] min-h-[56px] hover:bg-[var(--color-surface-hover)]/55">
          <input
            type="checkbox"
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-[7px]">
              <span className="truncate text-[13px] font-semibold text-[var(--color-text-primary)]">{project.name}</span>
              <span className="shrink-0 text-[11px] text-[var(--color-text-tertiary)]">
                {t('agentMigration.projectFiles', { count: project.itemIds.length })}
              </span>
              {!project.exists && <span className="shrink-0 text-[11px] font-semibold text-[var(--color-warning)]">{t('agentMigration.pathMissing')}</span>}
            </div>
            <p className="mt-[3px] truncate font-mono text-[11px] text-[var(--color-text-tertiary)]" title={project.path}>{project.path}</p>
          </div>
          <IconButton
...
function PathPair({ source, destination }: { source: string; destination: string }) {
  const t = useTranslation()
  return (
    <div className="grid gap-[8px] rounded-[8px] border border-[var(--color-border-separator)] bg-[var(--color-surface-container-low)] p-[12px] text-[11px] sm:grid-cols-[80px_minmax(0,1fr)]">
      <span className="font-semibold text-[var(--color-text-tertiary)]">{t('agentMigration.sourcePath')}</span>
      <code className="break-all text-[var(--color-text-secondary)]">{source}</code>
      <span className="font-semibold text-[var(--color-text-tertiary)]">{t('agentMigration.destinationPath')}</span>
