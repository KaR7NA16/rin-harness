...
export const sessionsApi = {
  /** Host 端会话内容搜索（@rin/memory/session-search 索引）。 */
  search(query: string) {
  ...
  exportSessions(body: { sessionIds?: string[]; projectPath?: string; includeAgentNotes?: boolean }) {
    return api.rawPost('/api/sessions/export', JSON.stringify(body)).then((buf) => {
  ...
  runBackup() {
    return api.post<{ ok: true; backup: BackupListEntry }>('/api/sessions/backup')
  },

  listBackups() {
    return api.get<{ backups: BackupListEntry[] }>('/api/sessions/backups')
  },

  restoreBackup(name: string) {
    return api.post<ImportSessionResult>('/api/sessions/backup/restore', { name })
  },

  getBackupSettings() {
    return api.get<{ settings: SessionBackupSettings }>('/api/sessions/backup-settings')
  },

  updateBackupSettings(settings: Partial<SessionBackupSettings>) {
    return api.put<{ settings: SessionBackupSettings }>('/api/sessions/backup-settings', { settings })
  },
}
...
export type SessionBackupSettings = {
  enabled?: boolean
  intervalDays?: number
  maxKeep?: number
  location?: string
