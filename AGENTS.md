...
Run checks before pushes using the relevant gates in [rin/AGENTS.md](rin/AGENTS.md) and [rin/docs/GATES-PLAN.md](rin/docs/GATES-PLAN.md); report only commands run. For registry dependency changes, also verify `pnpm install --frozen-lockfile` when applicable.
...
- `rin:test:coverage` is the progressive coverage gate for rin packages; [rin/AGENTS.md](rin/AGENTS.md) and [rin/docs/GATES-PLAN.md](rin/docs/GATES-PLAN.md) own its thresholds.

## Secrets / .env

Real-API tests and demos read `DEEPSEEK_API_KEY`, optional `DEEPSEEK_BASE_URL`, and root `.env`. cordis.yml allows `!!js` (never `!js`) under plugin `config` and entry `disabled`; other metadata stays literal, so conditional composition also uses overlays. Never commit credentials. CI e2e skips without a key; the current rin gate policy is documented in [rin/docs/GATES-PLAN.md](rin/docs/GATES-PLAN.md).
...
- **Waterfall listeners MUST call `next()`** to delegate; returning without it short-circuits the chain. The applicable rin-specific rules are maintained in [rin/AGENTS.md](rin/AGENTS.md).
- **Model-visible ⟺ logged**: anything that reaches a model request must be reconstructable from the session log; a new model-visible input requires a session event.
- **Plugins, not loop changes**: new behavior goes on documented extension points; changing `agent-loop` requires updating the current architecture notes in [rin/README.md](rin/README.md) and [rin/docs/README.md](rin/docs/README.md).
- **A capability seam comprises Service Definition / Service Provider / Consumer roles.** It is complete, never one role; split only when roles evolve independently. The rin seam matrix is maintained in [rin/docs/SEAM-PROJECTION.md](rin/docs/SEAM-PROJECTION.md).
...
- **Source plane vs artifact plane, never mixed.** Static gates and tests resolve workspace imports through tsconfig `paths` to `src` and pass on a clean tree; gates consuming built `lib/` declare that dependency. The current package layout is documented in [rin/AGENTS.md](rin/AGENTS.md).
