/**
 * Query the derived index: discover by FTS/LIKE, browse recent sessions, read
 * one session with head/tail truncation, and scroll around a message.
 *
...
function getSessionRow(db: SessionSearchDatabase, params: {
  sessionId: string
  projectPath?: string
}): SessionRow | null {
  if (params.projectPath) {
    const row = db
      ...
      .get(params.sessionId, params.projectPath) as SessionRow | undefined
    return row ?? null
  }
  const row = db
    .prepare(
...
function getMessagesForSession(db: SessionSearchDatabase, sessionKey: string): MessageRow[] {
  return db
    .prepare(
...
function getAnchoredHit(db: SessionSearchDatabase, match: MatchRow): SessionSearchHit | null {
  const session = db
    .prepare('SELECT * FROM sessions WHERE session_key = ?')
    .get(match.session_key) as SessionRow | undefined
...
export async function discoverSessionSearch(params: {
  ...
  roleFilter?: string[]
  db: SessionSearchDatabase
}): Promise<SessionDiscoverResult> {
  const query = params.query.trim()
  const limit = normalizeLimit(params.limit, 3, 10)
  const db = params.db
  let rows: MatchRow[] = []
  try {
    rows = searchWithFts({ ...params, query, limit })
  } catch {
    rows = searchWithLike({ ...params, query, limit })
  }
...
export async function readSessionSearch(params: {
  ...
  const rows = truncated
    ? [...messages.slice(0, head), ...messages.slice(-tail)]
    : messages
  return {
    success: true,
    mode: 'read',
...
export async function scrollSessionSearch(params: {
  ...
  window?: number
  db: SessionSearchDatabase
}): Promise<SessionScrollResult | null> {
  const db = params.db
  const session = getSessionRow(db, params)
  if (!session) return null
  const messages = getMessagesForSession(db, session.session_key)
...
export async function sessionSearch(params: {
  ...
  if (params.query?.trim()) {
    return discoverSessionSearch({
      ...
      ...(params.currentSessionId !== undefined ? { currentSessionId: params.currentSessionId } : {}),
      db: params.db,
    })
  }
