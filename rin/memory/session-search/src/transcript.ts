/**
 * Parse raw session JSONL into normalized, searchable transcripts without
 * filesystem access. Injected project-memory context, internal command
 * breadcrumbs, interruption notices, and empty responses are stripped so they
 * never reach the indexed corpus.
 *
 * @module @rin/session-search
 */

import { stripProjectMemoryContext } from './contextTag.ts'
import type { ParsedSessionTranscript } from './types.ts'
...
export function parseSessionTranscriptContent(params: {
  raw: string
  filePath: string
  projectPath: string
  sessionId: string
  ...
  const parsed = parseJsonl(params.raw)
  ...
  return {
    ...
    createdAt: firstTimestamp ?? params.fileBirthtime.toISOString(),
    modifiedAt: params.fileMtime.toISOString(),
    fileMtimeMs: params.fileMtimeMs,
