/**
 * Derive and index project memories from temporary sessions and external
 * memory files, with secret redaction, confidence scoring, and FTS search.
 *
...
export function redactProjectMemoryText(value: string): string {
  return value
...
function hasUsefulWorkDir(workDir: string | null, homeDir: string): boolean {
...
function buildProjectMemoryCandidate(
  ...
  const hasWorkDirSignal = hasUsefulWorkDir(parsed.workDir, homeDir)
...
export function deleteProjectMemoryBySessionKey(db: SessionSearchDatabase, key: string): void {
  const rows = db
    .prepare('SELECT id FROM project_memories WHERE session_key = ?')
    .all(key) as Array<{ id: number }>
  const deleteFts = db.prepare('DELETE FROM project_memories_fts WHERE rowid = ?')
  const deleteTrigram = db.prepare('DELETE FROM project_memories_fts_trigram WHERE rowid = ?')
  for (const row of rows) {
    deleteFts.run(row.id)
    deleteTrigram.run(row.id)
  }
  db.prepare('DELETE FROM project_memories WHERE session_key = ?').run(key)
}
...
export function upsertProjectMemoryForParsedSession(
  ...
  const candidate = buildProjectMemoryCandidate(parsed, options.homeDir)
  if (!candidate) {
  ...
  const existing = db
    .prepare('SELECT id, created_at FROM project_memories WHERE session_key = ?')
    .get(key) as { id: number; created_at: string } | undefined

  if (existing) {
    db.prepare('DELETE FROM project_memories_fts WHERE rowid = ?').run(existing.id)
    db.prepare('DELETE FROM project_memories_fts_trigram WHERE rowid = ?').run(existing.id)
  }
...
function searchProjectMemoriesWithFts(params: {
  ...
  const values: SQLInputValue[] = [ftsQuery]
...
function searchProjectMemoriesWithLike(params: {
  ...
  const values: SQLInputValue[] = [pattern, pattern, pattern, pattern]
  if (params.currentSessionId) {
