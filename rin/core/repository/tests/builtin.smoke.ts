...
import { resolveEnvironment } from '../../environment/src/plan.ts'
...
const { profile, packages } = resolveEnvironment(repo, 'scientific-base')
if (packages.length < 10) throw new Error('scientific-base resolution too small: ' + packages.length)
const bio = resolveEnvironment(repo, 'bioinformatics-base')
if (!bio.packages.some(p => p.id === 'python-biopython')) throw new Error('biopython missing after fix')
console.log('BUILTIN-SMOKE-OK', repo.environmentPackages.length, 'packages', bio.packages.length, 'bio pkgs', profile.metadata.id)
