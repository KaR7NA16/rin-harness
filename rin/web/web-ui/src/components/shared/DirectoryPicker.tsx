...
export function DirectoryPicker({ value, onChange, variant = 'default' }: Props) {
  ...
  const triggerClassName = variant === 'pill'
    ? 'flex h-[36px] items-center gap-[8px] rounded-full border border-[var(--color-border)] bg-[var(--color-surface-container-low)] px-[16px] text-[13px] font-bold leading-[16px] text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)]'
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {value ? (
        ...
        >
          {variant === 'pill' ? (
            selectedProject?.isGit ? (
              <GitBranch size={14} strokeWidth={1.75} className="shrink-0 text-[var(--color-text-tertiary)]" />
            ) : (
              <Folder size={14} strokeWidth={1.75} className="shrink-0 text-[var(--color-text-tertiary)]" />
            )
          ) : selectedProject?.isGit ? (
          ...
          <span className={`truncate ${variant === 'pill' ? 'max-w-[120px] text-[var(--color-text-primary)]' : 'max-w-[120px] text-[var(--color-text-primary)]'}`}>
            {selectedProject?.repoName || selectedProject?.projectName || value.split('/').pop()}
          </span>
          {selectedProject?.branch && (
            <span className={`truncate max-w-[72px] ${variant === 'pill' ? 'text-[var(--color-text-tertiary)]' : 'text-[var(--color-text-tertiary)]'}`}>{selectedProject.branch}</span>
          )}
          {variant === 'pill' ? (
        ...
        >
          {variant === 'pill' ? (
            <FolderOpen size={14} strokeWidth={1.75} className="shrink-0 text-[var(--color-text-tertiary)]" />
          ) : (
