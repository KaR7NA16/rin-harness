...
import { AGENT_COLOR_HEX } from '../../types/team'
...
function MemberRow({ member, onView }: { member: TeamMember; onView: () => void }) {
  ...
  const color = member.color ? AGENT_COLOR_HEX[member.color] : undefined
  ...
  return (
    ...
    >
      ...
      <Icon name="open_in_new" size={14} className="text-[var(--color-text-tertiary)] opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 transition-opacity" />
    </button>
  )
}
