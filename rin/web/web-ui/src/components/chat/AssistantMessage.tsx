      ...
      >
        <div className="relative w-full">
          <div
            ...
            className={
              layout === 'document'
                ? 'relative w-full rounded-[24px] rounded-bl-[8px] border border-[var(--color-border)] bg-[var(--color-message-assistant-bg)] px-[24px] py-[16px] text-[var(--color-text-primary)]'
                : useStableBubbleWidth
                  ? 'relative w-full max-w-[85%] rounded-[24px] rounded-bl-[8px] border border-[var(--color-border)] bg-[var(--color-message-assistant-bg)] px-[24px] py-[16px] text-[var(--color-text-primary)]'
                  : 'relative w-fit max-w-[85%] rounded-[24px] rounded-bl-[8px] border border-[var(--color-border)] bg-[var(--color-message-assistant-bg)] px-[24px] py-[16px] text-[var(--color-text-primary)]'
            }
          >
            {/* 左缘强调线: streaming 时呼吸 */}
            <span
              aria-hidden="true"
              className={`absolute left-[10px] top-[14px] bottom-[14px] w-[2px] rounded-full bg-[var(--color-text-accent)] ${
                isStreaming ? 'animate-accent-pulse-line' : 'opacity-40'
              }`}
            />
