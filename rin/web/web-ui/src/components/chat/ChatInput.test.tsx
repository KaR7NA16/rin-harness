...
describe('ChatInput composer controls', () => {
  ...
  it('sends /compact after confirming the compact dialog', () => {
    useChatStore.setState({
      sessions: {
        'session-1': makeChatSession(),
      },
    })
    const sendSpy = vi.spyOn(useChatStore.getState(), 'sendMessage').mockImplementation(() => {})
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Compact conversation context' }))
    expect(screen.getByText('Compact conversation?')).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: 'Compact' }))
    expect(sendSpy).toHaveBeenCalledWith('session-1', '/compact')
  })

  it('opens the session review view from the review button', () => {
    useChatStore.setState({
      sessions: {
        'session-1': makeChatSession(),
      },
    ...
    expect(screen.getByRole('dialog', { name: 'Session review' })).toBeInTheDocument()
  })
  ...
  it('opens the session review view when the highlighted /review slash command is submitted', async () => {
    useChatStore.setState({
      sessions: {
        'session-1': makeChatSession(),
      },
    ...
    render(<ChatInput onSubmit={onSubmit} sessionId="session-1" runtimeKey="session-1" />)
    ...
    expect(onSubmit).not.toHaveBeenCalledWith('/review', [])
    expect(screen.getByRole('dialog', { name: 'Session review' })).toBeInTheDocument()
  })
  ...
  it('shows a voice input button that reports it is unavailable', () => {
