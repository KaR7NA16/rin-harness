...
export function AskUserQuestion({ toolUseId, input, result, sessionId: sessionIdProp }: Props) {
  ...
  const submitted = Object.keys(resultAnswers).length > 0 || (hasSubmitted && !submitFailed)
  ...
  const handleSubmit = () => {
    ...
    if (!sessionId || !pendingRequest) return
    ...
    try {
      respondToPermission(sessionId, pendingRequest.requestId, true, {
        updatedInput: {
          ...inputObject,
          answers,
        },
      ...
      addToast({ type: 'error', message: t('question.submitFailed') })
    } finally {
  ...
  const handleDeny = () => {
    ...
    submittingRef.current = true
    try {
      respondToPermission(sessionId, pendingRequest.requestId, false)
    } catch {
      addToast({ type: 'error', message: t('question.submitFailed') })
    } finally {
      submittingRef.current = false
  ...
  const answeredCount = questions.filter((_, i) => Boolean(answersByIndex[i]?.value.trim())).length
  ...
  return (
    <div className={`mb-4 rounded-[var(--radius-lg)] border overflow-hidden ${
      ...
      {questions.length > 1 && (
        <div className="flex items-center px-4 border-b border-[var(--color-border-separator)] bg-[var(--color-surface-container-low)] overflow-x-auto">
          {questions.map((q, i) => {
          ...
          <span className="ml-auto flex shrink-0 items-center pl-4 text-[11px] font-medium text-[var(--color-text-tertiary)]">
            {t('question.progress', { answered: answeredCount, total: questions.length })}
          </span>
        </div>
      )}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className={`mt-0.5 flex-shrink-0 w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                      ...
                      {isSelected && (
                        <Icon name="check" size={10} className="text-[var(--color-on-primary)]" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
      ...
      {!submitted && (
        <div className="flex items-center gap-2 px-4 py-3 border-t border-[var(--color-border-separator)] bg-[var(--color-surface-container-low)]">
          {pendingRequest ? (
            ...
            <span className="text-[12px] text-[var(--color-text-tertiary)]">{t('question.expired')}</span>
          )}
        </div>
      )}
    </div>
