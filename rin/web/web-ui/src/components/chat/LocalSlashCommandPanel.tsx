  ...
  return (
    <div className="space-y-6">
      ...
      {usage.hasUnknownModelCost && (
        <div className="rounded-md border border-[var(--color-warning)]/20 bg-[var(--color-warning)]/10 px-4 py-3 text-[14px] text-[var(--color-warning)]">
...
function ContextStackedBar({ categories, rawMaxTokens }: { categories: ContextCategory[]; rawMaxTokens: number }) {
  ...
  return (
    <div className="overflow-hidden rounded-full bg-[var(--color-surface-container-high)]">
            ...
            >
              <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  <span className={`min-w-0 truncate text-[14px] font-semibold ${muted ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-primary)]'}`}>
                    {category.name}
                  </span>
                </div>
              ...
              <div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--color-surface-container-high)]">
                <div
                  className={muted ? 'h-full rounded-full opacity-65' : 'h-full rounded-full'}
                  style={{
                    width: `${Math.min(100, Math.max(0.5, percent))}%`,
                    backgroundColor: muted ? 'var(--color-text-tertiary)' : 'var(--color-text-accent)',
                  }}
                />
              </div>
            </div>
...
function InspectorStatusBadge({ status, t }: { status: string; t: Translate }) {
  ...
  const dotClass = isConnected ? 'bg-[var(--color-success)]' : isFailed ? 'bg-[var(--color-error)]' : 'bg-[var(--color-text-tertiary)]'
...
function McpServerIcon({ status }: { status: string }) {
  ...
  return <Icon name={icon} size={20} className={isFailed ? 'text-[var(--color-error)]' : 'text-[var(--color-success)]'} />
}
  ...
  return (
    <div className="rounded-md border-2 border-[var(--color-border)] bg-[var(--color-surface-container)] px-5 py-6">
      <div className="mb-8 flex items-start justify-between gap-4">
        ...
        <span className="rounded-sm border-2 border-[var(--color-border)] bg-[var(--color-surface-container-high)] px-2 py-1 font-mono text-[12px] text-[var(--color-text-secondary)]">{context.model}</span>
      </div>
      <div className="font-mono text-[24px] font-semibold text-[var(--color-text-primary)]">
        ...
        <span className="ml-3 align-middle text-[14px] font-normal text-[var(--color-text-accent)]">[{formatPercent(usedPercent)} {t('slash.inspector.context.used')}]</span>
      </div>
      <div className="mt-7">
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          detail={(
            <span>
              <span className="text-[var(--color-success)]">{t('slash.inspector.status.connected')}</span>
              <span className="mx-5 text-[var(--color-text-primary)]" />
              <span className="text-[var(--color-error)]">{t('slash.inspector.status.failed')}</span>
            </span>
          )}
        />
      ...
      <section>
        ...
        <KeyValueRows
          rows={[
            ...
            [t('slash.inspector.status.permissionMode'), <span className="rounded-sm bg-[var(--color-surface-container-high)] px-1.5 py-1">{data.status.permissionMode}</span>],
      ...
      {mcpServers.length > 0 && (
        <section>
          <InspectorSectionTitle
            action={<button type="button" onClick={onRefresh} className="font-mono text-[12px] tracking-[0.18em] text-[var(--color-text-accent)] hover:text-[var(--color-text-primary)]">↻ {t('slash.inspector.status.refresh')}</button>}
          >
            ...
            {mcpServers.map((server) => (
              <div
                ...
                className={`flex min-h-[48px] items-center justify-between gap-4 rounded-md border px-4 py-3 font-mono ${
                  server.status === 'failed' ? 'border-[var(--color-error)]/30 bg-[var(--color-error-container)]' : 'border-[var(--color-border)] bg-[var(--color-surface-container)]'
                }`}
              >
...
function SessionInspectorPanel({
  ...
  }, [sessionId, projectPath, t, refreshNonce])
  ...
  return (
    <SessionInspectorShell selectedTab={selectedTab} tabs={tabs} onSelectTab={setSelectedTab} onClose={onClose} t={t}>
      {error ? (
        ...
        <StatusTab data={data} commands={commands} onRefresh={() => setRefreshNonce((nonce) => nonce + 1)} t={t} />
      )}
