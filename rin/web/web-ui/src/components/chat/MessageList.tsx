...
export function MessageList({ sessionId, projectPath, isActive = true, bottomOverlayHeight = 0, __testInitialItemCount }: MessageListProps = {}) {
  ...
  const sessionTools = sessionState?.sessionTools ?? []
  ...
  // Precompute per-index user_text ordinal counts so getItemContent stays O(1)
  // per row instead of slicing/filtering the whole list for every visible row.
  const userMsgOrdinals = useMemo(() => {
    const before = new Int32Array(renderItems.length)
    const after = new Int32Array(renderItems.length)
    const isUserText = (item: RenderItem | undefined) =>
      item !== undefined && item.kind === 'message' && item.message.type === 'user_text' && !item.message.pending
    let count = 0
    for (let i = 0; i < renderItems.length; i++) {
      before[i] = count
      if (isUserText(renderItems[i])) count++
    }
    count = 0
  ...
  const getItemContent = useCallback(
    (index: number, item: RenderItem | undefined) => {
      ...
      const isRewindableUser = msg.type === 'user_text' && !msg.pending
      const rewindableUserIndex = isRewindableUser ? userMsgOrdinals.before[dataIndex] : null
      const rewindableUserOffsetFromEnd = isRewindableUser ? userMsgOrdinals.after[dataIndex] : null
  ...
  return (
    <div className="wechat-chat-bg scrollbar-no-track relative flex flex-1 flex-col overflow-hidden">
      {showEmptyOverlay && historyLoadState === 'error' && (
        <div className="mx-auto my-6 flex max-w-[420px] flex-col items-center gap-3 rounded-[10px] border-2 border-[var(--color-border)] bg-[var(--color-surface-container-low)] px-5 py-5 text-center">
          ...
          <button
            ...
            className="px-4 py-1.5 text-[12px] font-bold tracking-tight text-[var(--color-btn-primary-fg)] bg-[var(--color-btn-primary-bg)] rounded-[6px] shadow-[var(--shadow-button-primary)] hover:bg-[var(--color-btn-primary-bg-hover)] transition-colors"
          >
            {t('common.retry')}
          </button>
        </div>
      ...
      {historyLoadState === 'loaded' && messages.length === 0 && (
        <div className="mx-auto my-6 flex max-w-[420px] flex-col items-center gap-2 rounded-[10px] px-5 py-5 text-center">
          <div className="text-[13px] font-semibold text-[var(--color-text-primary)]">{t('chat.emptyTitle')}</div>
          <div className="text-[12px] text-[var(--color-text-tertiary)]">{t('chat.emptyHint')}</div>
        </div>
      )}
      ...
      {!showEmptyOverlay && (
        <Virtuoso
          ...
          components={{
            ...
            Header: () => <ListHeader isLoadingMoreHistory={isLoadingMoreHistory} sessionTools={sessionTools} />,
...
function ListHeader({ isLoadingMoreHistory, sessionTools }: { isLoadingMoreHistory: boolean; sessionTools: string[] }) {
  const t = useTranslation()
  return (
    <>
      {sessionTools.length > 0 && <InitializedSection tools={sessionTools} />}
