...
import { AGENT_LIFECYCLE_TYPES, agentColorHexForKey } from '../../types/team'
...
function AgentCallCard({
  ...
  const agentKey =
    ...
    description
  const agentColorHex = agentColorHexForKey(agentKey)
  ...
  return (
    ...
    >
      <div className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--color-surface-hover)]/50">
        <span
          aria-hidden
          className="shrink-0 h-2.5 w-2.5 rounded-full border-2 border-[var(--color-surface)]"
          ...
          {!expanded && !outputSummary && recentToolCalls.length > 0 && (
            <div className="mt-1 space-y-1">
              {recentToolCalls.map((recentToolCall) => (
                ...
                >
                  {formatRecentToolUseSummary(recentToolCall, resultMap, isRunning, t)}
                </div>
              ))}
            </div>
          )}
        ...
        <button
          ...
          aria-label={expanded ? t('chat.toolCall.collapseAgent') : t('chat.toolCall.expandAgent')}
        >
