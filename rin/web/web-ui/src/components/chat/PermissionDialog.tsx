...
const kbdHintClass = kbdChipClass

export function PermissionDialog({ requestId, toolName, input, description, sessionId: sessionIdProp }: Props) {
  const { respondToPermission } = useChatStore()
  const activeTabId = useTabStore((s) => s.activeTabId)
  const sessionId = sessionIdProp ?? activeTabId
  const pendingPermission = useChatStore((s) => sessionId ? s.sessions[sessionId]?.pendingPermission : undefined)
  const connectionState = useChatStore((s) => sessionId ? s.sessions[sessionId]?.connectionState : undefined)
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (key === '1' || key === 'y') {
      ...
      } else if ((key === '2' || key === 'a') && !isPlanApproval) {
        e.preventDefault()
        if (confirmAlways) {
          setConfirmAlways(false)
          respondToPermission(sessionId, requestId, true, { rule: 'always' })
        } else {
        ...
        e.preventDefault()
        setConfirmAlways(false)
        respondToPermission(sessionId, requestId, false)
      }
    }

    document.addEventListener('keydown', handler)
    return () => document.removeEventListener('keydown', handler)
  }, [isPending, sessionId, requestId, isPlanApproval, confirmAlways, respondToPermission])

  // Reset the always-allow confirmation when the request is resolved.
  useEffect(() => {
    if (!isPending) setConfirmAlways(false)
  }, [isPending])
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {/* Disconnected hint — actions queue until the socket returns */}
        {connectionState === 'disconnected' && (
          <div className="flex items-center gap-1.5 border-t border-[var(--color-border-separator)] px-3 py-1.5 text-[10px] text-[var(--color-warning)]">
            <Icon name="sync" size={10} className="animate-spin shrink-0" />
            {t('permission.disconnectedHint')}
          </div>
        )}

        {/* Always-allow confirmation */}
        {confirmAlways && (
          <div className="flex items-center justify-between gap-3 border-t border-[var(--color-border-separator)] bg-[var(--color-surface-container-low)] px-3 py-2">
            <p className="min-w-0 flex-1 text-[11px] leading-4 text-[var(--color-text-tertiary)]">
              {t('permission.confirmAlways', { toolName: meta.label })}
            </p>
            <div className="flex shrink-0 items-center gap-1.5">
              <button
                type="button"
                onClick={() => setConfirmAlways(false)}
                className="px-2.5 py-1 rounded-[var(--radius-sm)] text-[11px] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)] transition-colors cursor-pointer"
              >
                {t('common.cancel')}
              </button>
              <button
                type="button"
                autoFocus
                onClick={() => { setConfirmAlways(false); if (sessionId) respondToPermission(sessionId, requestId, true, { rule: 'always' }) }}
              ...
              >
                ...
                {t('permission.confirmAlwaysAction')}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5 border-t border-[var(--color-border-separator)] px-3 py-1.5">
          <button
            type="button"
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-[var(--radius-sm)] text-[11px] font-semibold text-[var(--color-btn-primary-fg)] bg-[var(--color-btn-primary-bg)] hover:bg-[var(--color-btn-primary-bg-hover)] transition-colors cursor-pointer"
            onClick={() => sessionId && respondToPermission(sessionId, requestId, true)}
          >
            <Icon name="check" size={12} />
            {isPlanApproval ? t('permission.planStart') : t('permission.allow')}
            <kbd className={kbdHintClass}>1</kbd>
          </button>
          {!isPlanApproval && (
            <button
              type="button"
              className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-[var(--radius-sm)] text-[11px] font-medium transition-colors cursor-pointer ${
                confirmAlways
                  ? 'bg-[var(--color-brand)]/12 text-[var(--color-brand)]'
                  : 'text-[var(--color-text-secondary)] bg-[var(--color-surface-container-low)] hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)]'
              }`}
              onClick={() => setConfirmAlways((v) => !v)}
            >
              <Icon name="verified" size={12} />
              {t('permission.allowForSession')}
              <kbd className={kbdHintClass}>2</kbd>
            </button>
          )}
          ...
          <button
            ...
            onClick={() => sessionId && respondToPermission(sessionId, requestId, false)}
          >
            <Icon name="close" size={12} />
            {isPlanApproval ? t('permission.planRevise') : t('permission.deny')}
            <kbd className={kbdHintClass}>3</kbd>
          </button>
        </div>
      </div>
    </div>
