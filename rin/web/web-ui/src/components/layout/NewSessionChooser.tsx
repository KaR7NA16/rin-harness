...
let cachedRecentProjectsAt = 0
const CACHE_TTL = 30_000

function readCachedRecentProjects(): RecentProject[] | null {
  if (!cachedRecentProjects) return null
  if (Date.now() - cachedRecentProjectsAt >= CACHE_TTL) return null
  return cachedRecentProjects
}

function writeCachedRecentProjects(projects: RecentProject[] | null) {
  cachedRecentProjects = projects
  cachedRecentProjectsAt = projects ? Date.now() : 0
}
...
export function NewSessionChooser({
  ...
  const [projects, setProjects] = useState<RecentProject[]>(() => readCachedRecentProjects() ?? [])
  const [isLoading, setIsLoading] = useState(() => readCachedRecentProjects() === null)
  const [creatingKey, setCreatingKey] = useState<string | null>(null)

  useEffect(() => {
    let alive = true
    const cached = readCachedRecentProjects()
    ...
    sessionsApi.getRecentProjects(8)
      .then(({ projects: nextProjects }) => {
        writeCachedRecentProjects(nextProjects)
  ...
  const handleCreate = async (key: string, input?: CreateSessionInput) => {
    ...
    try {
      ...
      if (ok) {
        writeCachedRecentProjects(null)
        onClose?.()
      }
    } catch (error) {
      console.error('[NewSessionChooser] Failed to create session:', error)
      addToast({
        type: 'error',
