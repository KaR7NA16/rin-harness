...
export function StartupErrorView({ error }: StartupErrorViewProps) {
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-[var(--color-surface)] px-6">
      <section className="w-full max-w-3xl rounded-[12px] border border-[var(--color-border)] bg-[var(--color-surface-container-low)] p-6 shadow-[var(--shadow-md)]">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              onClick={handleCopy}
            >
              {copyState === 'copied' ? t('app.copiedDiagnostics') : copyState === 'failed' ? t('app.copyFailed') : t('app.copyDiagnostics')}
            </Button>
            <Button
              type="button"
              variant="ghost"
