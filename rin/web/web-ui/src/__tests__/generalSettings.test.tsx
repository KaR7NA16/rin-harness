...
import { ExecutionBehaviorSettings, PermissionSettings, ProviderSettings, Settings } from '../pages/Settings'
...
describe('Settings > General tab', () => {
  ...
  it('keeps WebFetch preflight out of General and in Permissions', () => {
    render(<PermissionSettings />)

    const toggle = screen.getByLabelText('Run the domain safety preflight')
  ...
  it('lets the user change WebFetch preflight in Permissions', () => {
    render(<PermissionSettings />)
...
describe('Settings > Providers tab', () => {
  ...
  it('never places a masked saved API key into the editable key field', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Edit' }))
    const dialog = screen.getByRole('dialog')
    expect(within(dialog).getByPlaceholderText('Leave empty to keep current key')).toHaveValue('')
  })
})
