...
import { notifyWhenUnfocused } from '../lib/osNotifications'
...
// When the socket is not open, wsManager queues the outgoing message for the
// next reconnect instead of delivering it immediately. Without a hint, a user
// clicking Allow/Deny/Stop while offline would see the card disappear with no
// idea the action is deferred until the connection comes back.
function notifyQueuedWhenOffline(sessionId: string) {
  if (!wsManager.isConnected(sessionId)) {
    useUIStore.getState().addToast({
      type: 'info',
      message: t('chat.actionQueuedReconnect'),
    })
  }
}
...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  respondToPermission: (sessionId, requestId, allowed, options) => {
    ...
    notifyQueuedWhenOffline(sessionId)
  ...
  handleServerMessage: (sessionId, msg) => {
    ...
    switch (msg.type) {
      ...
      case 'permission_request':
        ...
        notifyWhenUnfocused(
          t('osNotify.permissionRequestTitle'),
          t('osNotify.permissionRequestBody', { toolName: msg.toolName }),
          `permission:${msg.requestId}`,
        )
        break
      ...
      case 'computer_use_permission_request':
        ...
        notifyWhenUnfocused(
          t('osNotify.permissionRequestTitle'),
          t('osNotify.permissionRequestBody', { toolName: 'ComputerUse' }),
          `computer-permission:${msg.requestId}`,
        )
        break
      ...
      case 'message_complete': {
        ...
        notifyWhenUnfocused(
          t('osNotify.assistantCompleteTitle'),
          t('osNotify.assistantCompleteBody'),
          'assistant-complete',
        )
        break
      }
      ...
      case 'system_notification':
        ...
        if (msg.subtype === 'init' && msg.data) {
          const data = msg.data as { model?: string; tools?: string[] }
          update(() => ({
            sessionTools: Array.isArray(data.tools) ? data.tools : [],
          }))
        }
