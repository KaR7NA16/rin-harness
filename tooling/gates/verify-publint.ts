 ...
 *
 * Run: node --import tsx/esm tooling/gates/verify-publint.ts
 */
import { resolve } from 'node:path'

// Publication checks run from the repository-level tooling entry point.
import { printResult, publintTargets, rinWorkspacePackages } from '../lib/publint-all.ts'
...
async function main(): Promise<number> {
  ...
  if (failed > 0) {
    console.error(`verify-publint: ${failed} workspace package(s) failed publint.`)
    return 1
  }
  console.log(`verify-publint: ${results.length} workspace package(s) pass publint.`)
  return 0
}
