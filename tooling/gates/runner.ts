...
/** A workspace verify leaf name, also the single-leaf mode argument. */
type LeafId = 'verify-deps' | 'verify-cordis' | 'verify-structure' | 'verify-readme' | 'verify-publint'

/** One workspace verify leaf, owned by a sibling gate agent. */
interface Leaf {
  id: LeafId
...
const LEAVES: readonly Leaf[] = [
  { id: 'verify-deps', script: 'tooling/gates/verify-deps.ts' },
  { id: 'verify-cordis', script: 'tooling/gates/verify-cordis.ts' },
  { id: 'verify-structure', script: 'tooling/gates/verify-structure.ts' },
  { id: 'verify-readme', script: 'tooling/gates/verify-readme.ts' },
  { id: 'verify-publint', script: 'tooling/gates/verify-publint.ts' },
]
...
async function main(args: string[]): Promise<number> {
  ...
  console.log(`quality-gates: running ${leaves.length} leaf script(s) for mode ${mode}.`)
...
function parseMode(raw: string | undefined): Mode {
  if (raw === undefined || raw === '') {
    throw new Error('quality-gates: expected check or a verify-* leaf id.')
  }
  if (raw === 'check' || LEAVES.some(leaf => leaf.id === raw)) return raw as Mode
  throw new Error(`quality-gates: unknown mode ${JSON.stringify(raw)}; expected check | verify-deps | verify-cordis | verify-structure | verify-readme | verify-publint.`)
}
...
function leafById(id: LeafId): Leaf {
  ...
  if (leaf === undefined) throw new Error(`quality-gates: no leaf registered for ${id}.`)
  return leaf
}
...
async function runLeaf(leaf: Leaf): Promise<LeafResult> {
  ...
  console.log(`quality-gates: start ${leaf.id}`)
...
function printLeaf(result: LeafResult): void {
  ...
  if (result.status === 'passed') {
    console.log(`quality-gates: PASS ${result.leaf.id} (${seconds}s)`)
    return
  }
  console.error(`quality-gates: FAIL ${result.leaf.id} (${seconds}s)`)
...
function printSummary(results: LeafResult[], durationMs: number): void {
  ...
  console.log(`\nquality-gates: ${passed} passed, ${failed} failed in ${seconds}s.`)
  const unsuccessful = results.filter(result => result.status === 'failed')
  if (unsuccessful.length === 0) return
  console.error('quality-gates: unsuccessful leaves:')
