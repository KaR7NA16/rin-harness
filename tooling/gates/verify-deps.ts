/**
 * Verify workspace dependency completeness: every `@rin/*` or
 * `@deepseek-ai/dsh-*` import in a workspace package's `src/` must be declared in
 ...
 * `@rin/memory/prompt` without listing it) without touching dsh upstream.
 *
 * Run: node --import tsx/esm tooling/gates/verify-deps.ts
 */
// Discovery is restricted to the canonical apps/* / packages/*/* roots.
...
interface Failure {
  /** Owning workspace package name (e.g. @rin/host/web-server). */
  packageName: string
  /** Repo-relative source path that imports the missing dependency. */
  file: string
  /** 1-based line of the import. */
  line: number
  /** Full import specifier as written (e.g. @rin/assets). */
  specifier: string
...
for (const manifestPath of globSync(manifestPatterns, { cwd: root }).sort()) {
...
if (failures.length > 0) {
  console.error(
    'verify-deps: source files import @rin/* / @deepseek-ai/dsh-* packages their manifest does not declare:',
  )
...
console.log(
  `verify-deps: ${packagesChecked} workspace package(s), ${sourceFiles} source file(s) checked; every @rin/* and @deepseek-ai/dsh-* import is declared.`,
)
