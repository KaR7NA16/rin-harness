...
} from '../../src/prompt/paths.ts'
import { ensurePromptMemorySeed } from '../../src/prompt/seed.ts'
import {
  DEFAULT_PROMPT_MEMORY_CONFIG,
  readPromptMemoryConfig,
  updatePromptMemoryConfig,
} from '../../src/prompt/config.ts'
import { PromptMemoryError, createPromptMemoryStore } from '../../src/prompt/store.ts'
import {
  appendPromptMemoryReviewLogs,
  getPromptMemoryReviewLogPath,
  readPromptMemoryReviewLogs,
} from '../../src/prompt/reviewLog.ts'
import { PROMPT_MEMORY_TOTAL_CHAR_LIMIT, type PromptMemoryAutoReviewLogEntry } from '../../src/prompt/types.ts'
...
describe('Prompt Memory persistence', () => {
  ...
  test('serializes entry mutations and rejects identity entry mutation', async () => {
    ...
    await expect(store.addEntry('soul', 'Not permitted.')).rejects.toBeInstanceOf(
      PromptMemoryError,
    )
  })
