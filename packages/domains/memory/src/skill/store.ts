/**
 * rin skill memory — file-backed memory store.
 *
 * createSkillMemoryStore(roots, options) owns file locking, atomic writes,
 * usage/stats consistency, summaries, and bounded pending/evidence retention.
 * It depends on no runtime, CLI, Tool, model, Server, or UI types. Runtime
 * command mapping, prompt formatting, fire-and-forget logging, lifecycle
 * scheduling, and model-driven review remain adapters outside this package.
 *
...
export interface SkillMemoryPendingEntry {
  version: 1
  id: string
  skillId: string
  rawKey: string
...
export interface SkillMemorySummary {
...
/**
 * Create a file-backed skill memory store for the injected config roots.
 *
 * @param roots - global and optional project config roots.
...
export function createSkillMemoryStore(
  ...
  const isFsInaccessible = (error: unknown): boolean => {
  ...
  async function acquireLock(lockPath: string): Promise<void> {
    ...
    throw new Error(`rin skill-memory: could not acquire skill memory lock: ${lockPath}`)
  }
  ...
  async function readJsonFile<T>(filePath: string, fallback: T): Promise<T> {
    ...
    try {
      ...
      // corrupt or unparseable file: fall back to the default record
      return fallback
    }
  }
  ...
  function getMemoryPath(
    ...
    return join(getSkillMemoryDir(ref, scope, roots), filename).normalize('NFC')
  }

  async function recordSkillLifecycleUsage(params: {
    ref: SkillMemoryRef
    scope: SkillMemoryScope
    status?: SkillLifecycleStatus
  }): Promise<void> {
    ...
    const sidecarPath = getSkillUsageSidecarPath(ref, scope, roots)
    ...
    await withFileLock(sidecarPath, async () => {
      ...
      usage.skills[skillId] = {
        ...
        ...(ref.loadedFrom === undefined ? {} : { loadedFrom: ref.loadedFrom }),
        status:
    ...
    await updateSkillMemoryStats(ref, scope, stats => ({
      ...
      reportError('rin skill-memory: failed to update usage stats', error)
    })
  }

  async function setSkillLifecycleStatus(params: {
    ref: SkillMemoryRef
    scope: SkillMemoryScope
    status: SkillLifecycleStatus
    ...
    await withFileLock(sidecarPath, async () => {
      ...
      reportError('rin skill-memory: failed to update usage status', error)
    })
  ...
  async function readSkillMemoryStats(
    ref: SkillMemoryRef,
    scope: SkillMemoryScope,
  ): Promise<SkillMemoryStats> {
    const skillId = getSkillMemoryId(ref)
  ...
  async function updateSkillMemoryStats(
    ref: SkillMemoryRef,
    scope: SkillMemoryScope,
  ...
  async function readSkillMemorySummary(
    ref: SkillMemoryRef,
    scope: SkillMemoryScope,
  ...
  async function readSkillMemorySummaries(
    ref: SkillMemoryRef,
  ...
  async function writeSkillMemorySummary(
    ref: SkillMemoryRef,
    scope: SkillMemoryScope,
    content: string,
  ): Promise<void> {
  ...
  async function appendSkillMemoryPending(
    ref: SkillMemoryRef,
    scope: SkillMemoryScope,
    ...
    await updateSkillMemoryStats(ref, scope, stats => ({
      ...
      // stats are best-effort; the pending entries are already durable
    })

    return fullEntry
  }

  async function readSkillMemoryPending(
    ref: SkillMemoryRef,
    scope: SkillMemoryScope,
  ...
  async function movePendingToEvidence(params: {
    ref: SkillMemoryRef
    scope: SkillMemoryScope
    ...
    await withFileLock(pendingPath, async () => {
      ...
      await updateSkillMemoryStats(ref, scope, stats => ({
        ...
        // stats are best-effort; the pending/evidence moves are already durable
      })
    })
  }
  ...
  async function completeReview(params: {
    ...
    if (params.summary !== undefined) {
      await writeSkillMemorySummary(params.ref, params.scope, params.summary)
    }
    await movePendingToEvidence({
      ref: params.ref,
      scope: params.scope,
      consumedCount: params.consumedCount,
    })
  }
...
export type SkillMemoryStore = ReturnType<typeof createSkillMemoryStore>
