 ...
 *
 * Wires the @rin/workspace/environment install-run state machine (create → approve →
...
import type { InstallExecutor, InstallRun } from '@rin/workspace/environment'
 ...
 * @rin/workspace/environment, which the strip-types smoke script must not resolve at
 * load time.
 *
...
export async function executeEnvironmentPlan(
  ...
  const { approveInstallRun, createInstallRun, executeInstallRun } = await import('@rin/workspace/environment')
 ...
 * @returns the executor handed to @rin/workspace/environment's executeInstallRun.
 */
