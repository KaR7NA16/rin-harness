...
capability probing, and dispatching a resolved @rin/workspace/environment install plan
...
`exec.ts` loads @rin/workspace/environment lazily, so the pure modules (including the
...
approved → provisioning/verifying → ready | failed) lives in @rin/workspace/environment; this
...
- **Install execution is not exercised end-to-end here.** `executeEnvironmentPlan`
  wires create → approve → execute over @rin/workspace/environment, routing stage commands
...
- **One executor per stage, not per command.** `buildStageExecutor` runs each
  ...
  result; @rin/workspace/environment records one audit-log entry per stage.
