...
a model, approve, and persist. It orchestrates `@rin/memory/skill` (the gate
and store) and `@rin/memory/prompt` (insight projection) without reimplementing
either.
...
- `../../domains/memory/src/skill/gate.ts` — the deterministic creation gate.
- `../../domains/memory/src/skill/paths.ts` — skill-memory layout for the overview.
- `../../domains/memory/src/prompt/{insights,store,reviewLog}.ts` — insight projection.
