/**
 * rin evolution — secret scan and redaction for skill candidates.
 *
 * Scans candidate markdown for credentials before approval so secrets never
 * reach the skill store, and redacts them from review excerpts before they
 * reach the review model. Rules are a curated subset of high-confidence
 * patterns from gitleaks (https://github.com/gitleaks/gitleaks, MIT license):
 * only rules with distinctive prefixes and near-zero false-positive rates.
 *
 * @module @rin/evolution
 */

export type SecretMatch = {
  /** Rule id that matched (e.g., "github-pat", "aws-access-token"). */
  ruleId: string
...
type SecretRule = {
  /** Rule id (kebab-case), used in labels. */
  id: string
  /** Regex source, lazily compiled on first scan. */
  source: string
  /** Optional regex flags (most rules are case-sensitive by default). */
  flags?: string
}
