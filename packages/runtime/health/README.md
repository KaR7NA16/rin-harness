...
The runtime-health package keeps the doctor and monitor Cordis plugins as
independent public subpaths: `@rin/health/doctor` and
`@rin/health/monitor`. The root entry exports both as namespaces.

## Monitor

`ctx.monitor.snapshot()` reports live CPU, memory, load, disk, uptime, and
platform data. It is mounted before doctor so diagnostics can include the same
honest point-in-time host metrics.
