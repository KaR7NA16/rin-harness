...
- 声明装配清单：`@deepseek-ai/dsh-base` + 当前 29 个 host 插件和 `@rin/host/web-server`，**不含 `@deepseek-ai/dsh-web-app`**（docs/archive/migration/MIGRATION.md §8 决策 10）。
...
- `RIN_WEB_SERVER`：`@rin/host/web-server`，默认监听 `127.0.0.1:8320`。

具体 entry、默认 Config 和 dsh storage overrides 以 `src/cordis.yml` 与
`cordis.patch.yml` 为准，`pnpm exec tsx tooling/gates/verify-cordis.ts`
会检查两条装配路径保持一致。

内置仓库根 `builtinRepositoryRoot()` 解析到 `packages/domains/assets/builtin/`；Web UI 静态根 `webUiDistRoot()` 解析到 `apps/web/dist/`。两者均相对本包解析，非相对进程 cwd。
...
```sh
pnpm run build:web   # = pnpm --filter @rin/web run build
pnpm run start       # 启动 host，WebView/浏览器打开 http://127.0.0.1:8320
```

开发迭代前端可另开 `pnpm --filter @rin/web run dev`（Vite dev server 代理到 8320）。
