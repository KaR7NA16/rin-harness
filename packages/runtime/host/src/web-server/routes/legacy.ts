...
import { isMonitorSupported } from '@rin/health/monitor'
...
export async function handle(
  ...
  // Search (legacy desktop path backed by @rin/memory/session-search)
  if (pathname === '/api/search/sessions') return searchSessionsRoute(method, body, services)

  // Skills (legacy desktop path backed by @rin/memory/skill)
  ...
  // Sandboxes (legacy desktop paths over @rin/workspace/sandboxes)
  ...
  // Agents (legacy desktop paths over @rin/workspace/agents)
 ...
 *
 * Mirrors @rin/host's builtinRepositoryRoot(): the builtin repository lives
 * at packages/domains/assets/builtin, outside this transitional package,
 * i.e. three above the package src/, the same relative depth the bundle
 * resolves from. Kept local because a project reference to @rin/host would
...
function builtinRepositoryRoot(): string {
  return fileURLToPath(new URL('../../../../../packages/domains/assets/builtin/', import.meta.url))
}
