...
> 历史快照：2026-08-16，审计基于迁移前的 `rin/` 目录布局。文中的包数量、文件路径、发布链路和整改状态不代表当前仓库；当前目录事实以 `docs/architecture/CURRENT.md` 与 `docs/audits/REPOSITORY-LAYOUT-CLOSEOUT.md` 为准。历史命中保留用于追溯，不得作为现行路径引用。
