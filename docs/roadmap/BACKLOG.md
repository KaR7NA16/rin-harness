...
## A. 知识一体化方案延后项（对应历史方案
[NOTES-KNOWLEDGE-INTEGRATION.md](../archive/migration/NOTES-KNOWLEDGE-INTEGRATION.md)）
...
## B. web-server / 产品面已知限制（`@rin/host/web-server` README）
...
## C. 产品待决（需外部凭据或 scope 决策，见
[CURRENT.md](../architecture/CURRENT.md)「待决」节）
...
> 本节只保留当前目录中尚未建立的包。`@rin/collaboration`、`@rin/health/doctor`、`@rin/computer-use`、
> `@rin/agent-migration`、`@rin/context/codegraph` 与 `@rin/automation` 已建成并接入当前装配；
> [MIGRATION.md](../archive/migration/MIGRATION.md) 的后置列表已同步移除这些项目。
