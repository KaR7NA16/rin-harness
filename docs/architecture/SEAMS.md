# 当前 seam projection 接入矩阵

> 状态：**Phase 9 已完成（2026-08-15，8 个子代理并行实现 + 主线程收口）**。
> 8 个缺口全部接入并通过 `pnpm run typecheck` + 9 个 seam 冒烟 + 5 个回归冒烟；
> 全部 11 个 @rin host 插件已投影进 dsh seam（knowledge/notes/token-optimization 此前已接入）。
> 本文档保留矩阵与 API 速查作为维护参考；当前执行路线见
> [ACTIVE.md](../roadmap/ACTIVE.md)，历史迁移证据见
> [MIGRATION.md](../archive/migration/MIGRATION.md)。
...
收口：`pnpm install --lockfile-only`（新增 dsh-tools/dsh-skill workspace 依赖）→ `pnpm run typecheck` 通过 → 9+5 冒烟全绿 → 统一提交。

## 1. 接入矩阵

> 表中的 prompt-memory、skill-memory、session-search、agents、environment、
> sandboxes、smart-pruning、repository 等是能力标签；它们现在分别位于
> `@rin/memory/*`、`@rin/workspace/*`、`@rin/context/*`、`@rin/assets`
> 等公开包或子路径中。矩阵描述 seam，不表示这些标签仍是独立 workspace package。

| # | 能力标签 | 目标 dsh seam | 当前状态 | 证据/源码位置 |
|---|---|---|---|---|
| 1 | prompt-memory | systemPrompt | 已接入 | `packages/domains/memory/src/prompt/seam.ts` |
| 2 | session-search | session 事件 + tools | 已接入 | `packages/domains/memory/src/session-search/seam.ts` |
| 3 | skill-memory | skills | 已接入 | `packages/domains/memory/src/skill/seam.ts` |
| 4 | agents | agentPresets | 已接入 | `packages/domains/workspace/src/agents/seam.ts` |
| 5 | environment + sandboxes | shell / subprocess | 已接入 | `packages/domains/workspace/src/sandboxes/seam.ts` |
| 6 | smart-pruning | toolResultPruner / compaction | 已接入 | `packages/features/context/src/smart-pruning/seam.ts` |
| 7 | evolution | llm / subagent | 已接入 | `packages/features/evolution/src/seam.ts` |
| 8 | repository | tools | 已接入 | `packages/domains/assets/src/seam.ts` |
| — | knowledge | tools | 已接入 | `packages/domains/knowledge/src/knowledge/index.ts` |
| — | notes | tools | 已接入 | `packages/domains/notes/src/index.ts` |
| — | token-optimization | systemPrompt | 已接入 | `packages/features/context/src/token-optimization/store-core.ts` |

“已接入”只表示 seam 注册代码存在并由当前结构/依赖/Cordis/smoke 相关门禁覆盖；
它不替代 provider、真机 host boot 或桌面发布证据。当前执行路线见
[ACTIVE.md](../roadmap/ACTIVE.md)。

## 2. dsh seam API 速查（实现时引用）

### 2.1 tools — ctx.tools.register(defineTool({...}))
- 包：@deepseek-ai/dsh-tools（**真实依赖**，范例：packages/domains/knowledge/src/knowledge/index.ts、packages/domains/notes/src/index.ts、packages/session-query/tool-session-query/src/index.ts）
- 字段：name / description / parameters / output.schema(+render) / execute / presentCall

### 2.2 systemPrompt — section + assemble
- 结构型 seam（不依赖 dsh 包），范例：packages/features/context/src/token-optimization/store-core.ts 的 TokenOptimizationSeam
...
- 不改 package.json 的包：主线程跑 pnpm run typecheck 收口。
...
- **收口（主线程）**：评审（seam 三角齐全、冒烟通过、README 门禁）→ pnpm install --lockfile-only → pnpm run typecheck → 统一提交。
